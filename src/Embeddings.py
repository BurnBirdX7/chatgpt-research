...
class Embeddings:
    def __init__(self, tokenizer: RobertaTokenizer, model: RobertaModel):
    ...
    def from_ids(self, input_ids: List[int]) -> np.ndarray:
        ...
        """
        sequence_length: int = self.model.config.max_position_embeddings
        shorten_by: int = 2 if sequence_length % 2 == 0 else 3
        sequence_length -= shorten_by
        window_step: int = sequence_length // 2

        embedding_len: int = self.model.config.hidden_size
